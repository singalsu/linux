			  ...
			  unsigned int size)
{
	...
	printk(KERN_DEBUG "At snd_sof_bytes_ext_put");
	...
	printk(KERN_DEBUG "Completed pm_runtime_get_sync");

	printk(KERN_DEBUG "Bytes ptr addr = %d", bytes);
	printk(KERN_DEBUG "Data size = %d", bytes[1]);
	if (bytes[1] > be->max) {
		dev_err(sdev->dev, "error: size too big %d bytes max is %d\n",
			bytes[1], be->max);
		ret = -EINVAL;
	...
	printk(KERN_DEBUG "Completed data size");
	...
	printk(KERN_DEBUG "Completed dev_dbg");
